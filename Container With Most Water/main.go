...
func getMax(v, v2, k, k2 int) int {
	...
	//fmt.Println("minV:", minV, " l:", l)
	return minV * l
}
func maxArea(ah []int) (max int) {
	for k, v := range ah {
		for i := k; i < len(ah); i++ {
			if max < getMax(v, ah[i], k, i) {
				max = getMax(v, ah[i], k, i)
			}
		}
	}
	return
